          ...
          <div className="hidden lg:block absolute right-4 top-8 sticker-pop">
            <div className="relative">
              ...
              <div className="h-32 w-32 rounded-full grid place-items-center">
                <img src={logo} className="h-[54px] w-auto object-contain" style={{ transform: "rotate(8deg) translate(5.8%, -4.6%)" }} alt="Medscale Systems Logo" />
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-7 lg:pl-12">
            ...
            <div className="mt-10 border-t border-ink/10 pt-6 flex items-start gap-4">
              <div className="flex -space-x-2">
                {["SJ", "MC", "AR", "+"].map((i, k) => (
                  <span key={k} className={`grid place-items-center h-9 w-9 rounded-full text-[10px] font-semibold border-2 border-bone ${k === 3 ? "bg-[#0000cd] text-[#0a0a0a]" : "bg-[#0a0a0a] text-[#f5f1ea]"}`} style={k === 3 ? {} : { color: "#f5f1ea" }}>
                    {i}
                  </span>
                ))}
              </div>
